    ...
    <GlassCard as="section" className="xl:col-span-4 flex h-full flex-col p-6">
      <div className="flex h-full flex-col gap-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-widest text-white/40">US + CN indices</p>
            <h2 className="mt-2 text-xl font-semibold text-white">Index Trends</h2>
            <p className="mt-1 text-sm text-white/55">Cross-market tape compressed into a three-column scan surface.</p>
          </div>
            ...
            return (
              <article key={item.symbol} className="min-w-0">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className={cn('shrink-0 text-[11px] font-bold', tone)}>{formatChangeSummary(item)}</span>
                </div>
