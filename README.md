...
**Web 登录保护**：生产、Docker、云服务器部署必须在 `.env` 中保持 `ADMIN_AUTH_ENABLED=true`。本地开发如需无密码调试可临时关闭。首次访问在网页设置初始密码，保护 Settings 中的 API 密钥等敏感配置。系统设置现支持运行时开启或关闭认证；关闭认证不会删除已保存密码，后续可直接重新启用。认证开启时，`POST /api/v1/auth/logout` 也需要有效会话；如果会话已经过期，前端会直接回到登录页。详见 [完整指南](docs/full-guide.md)。
