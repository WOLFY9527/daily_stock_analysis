...
- 🔐 **Auth/admin/provider recovery pass with live-browser limits recorded** — Web logout confirmation now returns to `/login` after clearing local session state, with unit coverage for the redirect path; `/reset-password` now calls the new non-enumerating reset request endpoint and links back to localized login redirects. `/settings/system` also exposes the advanced AI provider/channel editor for built-in and custom API providers, including add/edit/delete, connection checks, Quick API status, and advanced-channel counts on provider cards. Live browser validation covered Safari Admin Mode entry, system settings navigation, AI provider drawer visibility, and EN/ZH switching; full logout, reset-password, and ordinary-user `/settings/system` access remain environment-limited while local `authEnabled=false`.
