...
- 🧭 **Rule backtest support export index API** — `GET /api/v1/backtest/rule/runs/{run_id}/export-index` 现在会返回单条规则回测的紧凑 export discovery/index。当前索引只列出最小稳定导出集合：`support_bundle_manifest_json`、`execution_trace_json`、`execution_trace_csv`，并显式给出 `available`、`availability_reason`、`format`、`media_type`、`delivery_mode`、`endpoint_path`（若存在）以及 `payload_class`。其中 manifest 会暴露现有 API path；execution-trace 两个导出当前仍是 service/file helper，因此 index 只会返回 `service_file_export` 且不会伪造下载路由。trace availability 直接按当前 resolved `execution_trace.rows` 是否可导出判断，缺失时会稳定返回 `execution_trace_rows_missing`。
