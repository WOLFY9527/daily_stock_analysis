...
- 🧮 **Rule backtest compare-runs backend foundation** — 新增 `POST /api/v1/backtest/rule/compare`，用于基于已持久化的规则回测结果做 stored-first 的首版 run-to-run comparison，不会重新执行回测。当前 compare contract 只暴露第一批可信字段：run metadata / date range、`parsed_strategy` 摘要、核心 metrics、benchmark/buy-and-hold summary、`execution_model` 高层摘要，以及每条运行自己的 `result_authority`。响应同时显式返回 `requested_run_ids` / `resolved_run_ids` / `comparable_run_ids` / `missing_run_ids` / `unavailable_runs`，便于区分“已完成可对比”“请求缺失”“运行尚未完成”这几类状态。
