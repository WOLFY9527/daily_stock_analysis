...
- 💬 **问股 Chat 重构为全屏悬浮 AI 视窗** — `apps/dsa-web/src/pages/ChatPage.tsx` 彻底拆掉了旧的常规文档流聊天骨架，改成真正的 full-bleed 100vh 画布：消息流现在独立滚动并放宽到 `max-w-5xl`，底部为绝对定位的渐变吸底输入层，原先散落在页面中段的技能模式切换被收口到输入框上方的控制台/插件栏，空状态也改成更有呼吸感的垂直居中布局。此次改动保持会话历史、流式回复、追问上下文、导出/发送动作与 agent API 不变，只重构问股页面的信息架构与交互壳层。
