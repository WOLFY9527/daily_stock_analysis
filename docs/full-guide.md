...
- 登录页现在新增“忘记密码？”入口与 `/reset-password` 请求页；该页面只接受用户名/邮箱并触发后端 reset request API，返回统一成功提示以避免暴露账户是否存在。
- AI 高级渠道编辑器现在支持维护附加请求头/参数 JSON，并在删除唯一来源渠道时同步清理失效的 runtime model / fallback / vision 引用，避免设置页残留悬空渠道路由。
