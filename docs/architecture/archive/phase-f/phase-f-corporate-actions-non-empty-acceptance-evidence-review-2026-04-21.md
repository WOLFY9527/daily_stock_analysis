# Phase F Corporate-Actions Non-Empty Acceptance-Evidence Review

## Goal

Document the bounded non-empty comparison-only evidence collected for corporate-actions on a real local PostgreSQL store and evaluate whether the current line is now past empty-only evidence.

This document is docs-only and review-only. It does not authorize or implement PostgreSQL serving for `GET /api/v1/portfolio/corporate-actions`.
...
- [phase-f-corporate-actions-non-empty-evidence-collection-plan-2026-04-21.md](/Users/yehengli/daily_stock_analysis/docs/architecture/phase-f-corporate-actions-non-empty-evidence-collection-plan-2026-04-21.md)
...
4. simple supported filter
   - `account_id=1&action_type=cash_dividend&page=1&page_size=20`

No broader request matrix was sampled.

## 2. Exact Non-Empty Evidence Observed

The focused real-PG run uses three seeded corporate-action rows for allowlisted `account_id=1`.
...
- `action_type=cash_dividend&page=1&page_size=20` matched with ordered ids `[3, 1]`
...
## 3. Diagnostic Evidence Surface

For the current corporate-actions line, the bounded reviewer evidence is the collected request-local diagnostic report set.

That report set showed:

- `comparison_source = "phase_f_pg_corporate_actions_candidate"` for every sampled request
- non-empty `legacy_summary.ordered_ids` for every sampled request
- matching non-empty `pg_summary.ordered_ids` for every sampled request
- `pg_source_available = true` for every sampled request
- `mismatch_class = null` for every sampled request

The current corporate-actions line does not yet expose a separate compact evidence summary helper. That is a tooling difference from cash-ledger, not a sign that the comparison path is still scaffolding-only.

## 4. What This Changes

This pass answers the current corporate-actions evidence question clearly:

- corporate-actions is no longer limited to empty-only comparison evidence
...
For the bounded corporate-actions comparison-only line, the evidence state is now:
...
- broader database migration completeness
- replay readiness
- snapshot readiness
