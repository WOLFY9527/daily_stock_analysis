...
This manual covers the helper-consolidation maintenance work around:
...
- keep the stored-first US history path readable and debuggable after wrapper removal
- document the consolidated Phase F comparison helpers
- make validation and rollback repeatable for AI-assisted and human operators

## Consolidated Helpers

### `PortfolioService._normalize_phase_f_compare_value`

- Purpose: normalize Phase F comparison values before equality checks so representation-only drift on `created_at` does not produce false-positive blocking mismatches.
- Current contract:
  - fields other than `created_at` are returned unchanged
  - `created_at` accepts `datetime`, ISO-like `str`, or `None`
  - timezone-only drift is normalized by dropping `tzinfo` while preserving wall-clock text
  - invalid datetime strings stay unchanged so real payload drift remains visible
...
- This helper is intentionally payload-light and safe for report collection.
...
- historical-evaluation fill path in `run_backtest()`
- sample warmup path in `prepare_backtest_samples()`

Debug intent:

- if a US historical-evaluation run cannot find `start_daily` or enough forward bars, inspect whether `fetch_daily_history_with_local_us_fallback` was called with:
...
- This manual is maintenance-focused and intentionally lives under `docs/` rather than `README.md`.
