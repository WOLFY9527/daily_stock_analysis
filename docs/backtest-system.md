...
- 当前 deterministic indicator strategies 在既有 `均线交叉 / MACD / RSI` 主信号之外，新增支持 additive 的 `fixed stop-loss` 扩展；当自然语言里出现 `止损 5%` 这类固定百分比止损时，解析结果会保留 `parsed_strategy.strategy_spec.risk_controls.stop_loss_pct`，执行时按“收盘触发、下一根 bar 开盘离场”的既有成交语义运行。`止盈` 与 `trailing stop` 仍保持 unsupported。
