...
- `GET /api/v1/backtest/rule/runs/{run_id}/export-index` 提供单条规则回测当前可发现的导出项索引，当前只列出最小稳定集合：`support_bundle_manifest_json`、`execution_trace_json`、`execution_trace_csv`。其中 manifest 会给出可直接读取的 API path；execution-trace 导出则只返回 `service_file_export` 交付方式与当前 availability，不会伪造尚未存在的下载路由。trace 可用性按当前 detail 读回后的 `execution_trace.rows` 是否非空判断，因此当历史运行缺少可导出的 trace rows 时，index 会稳定返回 `available=false` 与 `execution_trace_rows_missing`，便于脚本先做 discovery 再决定是否请求 heavier export。
