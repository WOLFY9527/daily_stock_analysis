...
- `GET /api/v1/backtest/rule/runs/{run_id}/export-index` returns the compact discovery index for the currently exportable artifacts of one rule-backtest run. The current stable set is intentionally small: `support_bundle_manifest_json`, `execution_trace_json`, and `execution_trace_csv`. The manifest entry includes its readable API path directly; the execution-trace entries only report `service_file_export` delivery plus truthful availability instead of inventing download routes that do not exist yet. Trace availability is determined from the resolved detail readback `execution_trace.rows`, so older runs without exportable trace rows return `available=false` with `execution_trace_rows_missing`.
