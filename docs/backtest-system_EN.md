...
- Deterministic indicator strategies now support one additive advanced-order primitive on top of the existing `moving_average_crossover / macd_crossover / rsi_threshold` signal families: fixed stop-loss. When the natural-language strategy includes a bounded percentage such as `stop loss 5%`, the parsed payload now preserves `parsed_strategy.strategy_spec.risk_controls.stop_loss_pct`, and execution follows the existing semantics of “signal on close, exit on the next bar open.” `take profit` and `trailing stop` remain unsupported.
