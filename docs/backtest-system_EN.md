...
- `POST /api/v1/backtest/rule/compare` is the stored-first compare-runs read path: it only reads already-persisted completed runs, does not re-execute backtests, and currently returns the smallest trustworthy comparison surface across metadata, `parsed_strategy`, core metrics, benchmark summary, `execution_model`, each run's `result_authority`, plus an additive `comparison_summary`. The current `comparison_summary` always picks the first comparable run in request order as the baseline and only emits deltas/comparability diagnostics for a narrow trusted metric set: `total_return_pct`, `annualized_return_pct`, `max_drawdown_pct`, `benchmark_return_pct`, and `excess_return_vs_benchmark_pct`.
