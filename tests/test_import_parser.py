...
class TestParseImportFromBytesCsv:
    ...
    def test_parses_explicit_hk_symbol_to_the_canonical_identity(self):
        data = "code,name\n0700.HK,腾讯控股".encode("utf-8")
        result = parse_import_from_bytes(data, "a.csv")
        assert result == [("HK00700", "腾讯控股", "medium")]
...
class TestParseImportFromText:
    ...
    def test_parses_single_column_codes(self):
        ...
        result = parse_import_from_text(text)
        assert len(result) == 2
        assert result[0] == (None, "00700", "medium")
