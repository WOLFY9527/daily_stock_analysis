...
class MarketCnIndicesApiTestCase(unittest.TestCase):
    ...
    def test_cn_indices_endpoint_returns_stable_contract(self) -> None:
        ...
        self.assertIn(payload["freshness"], {"live", "delayed", "cached", "stale", "partial", "fallback", "mock", "error", "unavailable"})
        ...
        self.assertTrue(first_item["value"] is None or isinstance(first_item["value"], (int, float)))
        if payload.get("isUnavailable"):
            self.assertIsNone(first_item["value"])
            self.assertIsNone(first_item["asOf"])
    ...
    def test_cn_indices_fallback_is_not_empty_when_provider_fails(self) -> None:
        ...
        self.assertTrue(all(item["price"] is None for item in payload["items"]))
        ...
        self.assertTrue(all(item["warning"] == "真实市场观察暂不可用，未显示示例数值" for item in payload["items"]))
    ...
    def test_cn_indices_supports_mixed_item_level_metadata(self) -> None:
        ...
        self.assertIsNotNone(live_item["value"])
        self.assertIsNotNone(live_item["asOf"])
        self.assertEqual(fallback_item["source"], "unavailable")
        self.assertEqual(fallback_item["freshness"], "unavailable")
        self.assertFalse(fallback_item["isFallback"])
        self.assertTrue(fallback_item["isUnavailable"])
        self.assertIsNone(fallback_item["asOf"])
        self.assertIsNone(fallback_item["value"])
        self.assertIsNone(fallback_item["price"])
        self.assertIsNone(fallback_item["change"])
        self.assertIsNone(fallback_item["changePercent"])
        self.assertEqual(fallback_item["sparkline"], [])
        self.assertEqual(fallback_item["trend"], [])
    ...
    def test_cn_indices_service_owns_public_metadata_after_sina_transport_rows(self) -> None:
        ...
        self.assertNotEqual(payload["freshness"], "live")
        self.assertTrue(payload["fallbackUsed"])
        self.assertEqual(payload["warning"], "真实市场观察暂不可用，未显示示例数值")
        self.assertFalse(payload["isFallback"])
        self.assertTrue(payload["isPartial"])
    ...
    def test_cn_indices_projects_akshare_observation_coverage_without_appending_rows(self) -> None:
        ...
        fallback_item = next(item for item in payload["items"] if item["symbol"] == "000300.SH")
        self.assertEqual(fallback_item["source"], "unavailable")
        self.assertTrue(fallback_item["isUnavailable"])
        self.assertIsNone(fallback_item["asOf"])
        self.assertIsNone(fallback_item["value"])
    ...
    def test_cn_indices_test_reset_ignores_leaked_persistent_snapshot_state(self) -> None:
        ...
        try:
            ...
            self.assertEqual(isolated["source"], "unavailable")
            self.assertTrue(all(item["isFallback"] is False for item in isolated["items"]))
            self.assertTrue(all(item["isUnavailable"] for item in isolated["items"]))
            self.assertTrue(all(item["value"] is None for item in isolated["items"]))
        finally:
